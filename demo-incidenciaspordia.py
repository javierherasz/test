...
fecha_ini = dateutil.parser.parse("2016-07-01T22:00:00Z")
fecha_fin = dateutil.parser.parse("2016-09-13T22:00:00Z")
...
# Limpio el MySQL
query = """DROP MEASUREMENT incidenciaspordia"""
try:
    influx.query(query)
except:
    pass
...
for dt in dategenerator(fecha_ini, fecha_fin):
    ...
    day_ini = datetime(yesterday.year, yesterday.month, yesterday.day, 22, 00, 00, 999999)
    day_fin = datetime(dt.year, dt.month, dt.day, 22, 00, 00, 999999)
    ...
    filt = {"$and" :[{"initial_date": {"$gte" : day_ini, "$lte" : day_fin}}, {"type" : "Claim"}, {"status" : { "$ne" : "Canceled"}}, {"customerservice" : "CS_SM2MS"}]}
