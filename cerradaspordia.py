...
# Limpio el MySQL
query = """DROP MEASUREMENT cerradaspordia"""
try:
    influx.query(query)
except:
    pass
...
for dt in dategenerator(fecha_ini, fecha_fin):
    ...
    filt = {"$and" :[{"restoration_date" : {"$gte" : day_ini, "$lte" : day_fin}}, {"type" : "Claim"}, {"status" : "Closed"}, {"customerservice" : "CS_SM2MS"}]}
  # Para la fecha actual, procesamos todo el mongo
    ...
    # Obtengo el contacto TAG
      ...
      for change in doc['status_change']:
            if change['code'] in "Closed":
                fecha_cerrada = change['start']
      try:
          tiempo_enrestaurar = fecha_cerrada - fecha_apertura
      except:
          pass
  ...
  ##### Nuevo
      ...
      if isinstance(fecha_cerrada, datetime):
          fecha_restaurado_epoch = int(fecha_cerrada.strftime('%s'))
      else:
      ...
      if fecha_cerrada >= fecha_ini and fecha_cerrada <= fecha_fin:
