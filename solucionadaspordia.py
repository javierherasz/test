...
# Limpio el MySQL
query = """DROP MEASUREMENT solucionadaspordia"""
try:
    influx.query(query)
except:
    pass
...
for dt in dategenerator(fecha_ini, fecha_fin):
    ...
    filt = {"$and" :[{"restoration_date" : {"$gte" : day_ini, "$lte" : day_fin}}, {"type" : "Claim"}, {"status" : "Solved"}, {"customerservice" : "CS_SM2MS"}]}
  ...
  ##### Nuevo
      ...
      if fecha_restaurado >= fecha_ini and fecha_restaurado <= fecha_fin:

          if status == 'Solved':
            solucionadas=solucionadas+1
      


    # Datos para influxdb
    influxdata = {
                  "measurement": "solucionadaspordia",
